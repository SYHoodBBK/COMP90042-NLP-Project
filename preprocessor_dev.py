...
def process_claim_item(item):
    ...
    if "claim_label" in claim_data:
        processed["claim_label"] = claim_data["claim_label"]
    if "evidences" in claim_data:
        processed["evidences"] = claim_data["evidences"]

    return claim_id, processed
...
def process_claims_parallel(claims: dict, num_workers: int = None):
    ...
    with Pool(processes=num_workers, initializer=init_worker) as pool:
        results = list(tqdm(pool.imap(process_claim_item, claims.items()),
                            total=len(claims), desc="Processing claims"))

    processed_claims = {cid: processed for cid, processed in results}
    return processed_claims

def main():
    input_path = 'data/dev-claims.json'
    output_path = 'data/processed_dev_claims.json'
